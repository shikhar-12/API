    ...
    try {
      ...
      if (record) {
        ...
        if (ismatched) {
          if (np === cnp) {
            ...
            res
              ...
              .json({
                status: "Failed",
                message: "New Password and Confirm Password are matched",
              });
          }
        } else {
          res
        ...
        res
          ...
          .json({
            status: "Failed",
            message: "Record Not Found Please Check Your old password",
          });
      }
    } catch (error) {
      console.log(error);
  ...
  static updateprofile = async (req, res) => {
    try {
      ...
      if (!req.files) {
        ...
        res
          .status(201)
          .json({ status: "success", message: "only Name and Email Updated" });
      } else {
        ...
        res
          ...
          .json({
            status: "success",
            message: "Name, Email and Image Updated",
          });
      }
    } catch (error) {
      console.log(error);
