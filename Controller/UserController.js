...
class UserController{
 ...
 res.status(201).json({status: "success",message: "good",records : result});
// res.send('hello api');
    }
    catch(error){
console.log(error);
...
console.log(error);
  ...
  static userinsert =  async (req,res)=>{
    try{
            ...
            if(rty)
            {
              res.status(401).json({status: "failed",message: "email already taken"});
            }
            else{
              if(un && em && pw && cpw)
              {
              ...
              const op = await cloudinary.uploader.upload(img.tempFilePath,{folder:"apiproject"});
            // console.log(op);
              ...
              res.status(201).json({status: "success",message: "good"});
                  ...
                  res.status(401).json({status: "failedd",message: "Password do not match"});
                 }
              }
              else{
    ...
    try{
      ...
      if(em && pw)
      {
        ...
        if(ismatched)
        {
          ...
          res.status(201).json({status:"Success",message:"Logged in"});
        }
        else{
          ...
          res.status(401).json({status:"Failed",message:"Record Do Not Exist"});
        }
      }
      else{
        res.status(401).json({status:"Failed",message:"Fill all Fields"});
